...
if [ -e $FILE ]; then 
	...
	if [ -n "$1" ] && [ -n "$2" ] & [ -n "$3" ] & [ -n "$4" ]  ; then
			...
			if [ $sauceenv == "Web" ]; then
				...
				sleep $delay
                		ulimit -n 8000; cd $MYDIR; bin/sc -u $1 -k $2 -i $4 -v -l $LOGFILE
			fi

			if [ $sauceenv == "EU" ]; then
                                echo "Create a tunnel on EU RDC"
				sleep $delay
		 		ulimit -n 8000; cd $MYDIR/bin; ./sc -u $1 -k $2 -x $DCURLEU -i $4 -v -l $LOGFILE
                        fi
			...
			if [ $sauceenv == "US" ]; then
				...
				sleep $delay
				ulimit -n 8000; cd $MYDIR ; bin/sc -u $1 -k $2 -x $DCURLUS  -i $4 -v -l $LOGFILE 
                        fi
			...
			if [ $sauceenv == "HEADLESS" ]; then
				...
				echo "Headless endpoint: " $HEADLESSURL
                                sleep $delay
				ulimit -n 8000; cd $MYDIR; bin/sc -u $1 -k $2 -x https://us-east-1.saucelabs.com/rest/v1 -i $4
